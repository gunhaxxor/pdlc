  ...
  Struct.prototype = /** @lends Struct# */ {
    ...
    read: function (arrayBuffer, offset) {
      ...
      var that = this,
        ...
        dv =
          (arrayBuffer instanceof DataView && arrayBuffer) ||
      ...
      var readed = align(
        ...
        function (item, prop, struct) {
          ...
          if (arrayBuffer.byteLength <= that.offset) {
            return new Error(
              "Uncaught IndexSizeError: Index or size was negative."
            );
          }
          ...
          if (vary === true) {
            try {
              ...
              length =
                dv["get" + capitalize(lengthTyped)](that.offset, endianness) /
                size;
            } catch (e) {
              console.error(
                e,
            ...
            while (i < length) {
              if (isStruct(typed)) {
                ...
                if (that.offset + size > arrayBuffer.byteLength) {
                  ...
                  values[i] = dv["get" + capitalize(typed)](
                    that.offset,
                    endianness
                  );
                }
    ...
    write: function (struct) {
      ...
      this._debug &&
        console.info("STRUCT.WRITE", "byteLength:", this.byteLength);
      ...
      align(this.struct, function (item, prop) {
        ...
        if (vary === true) {
          dataView["set" + capitalize(lengthTyped)](
            offset,
            length * size,
            endianness
          );
          ...
          that._struct[prop + "Size"] = [lengthTyped, length * size];
          that._debug &&
            console.log(prop + "Size", that._struct[prop + "Size"], offset);
        }
        ...
        if (
          ...
          values =
            (value && isString(value) && strToCharCodeArr(value)) || values;
        } else {
          values = [value];
        }
      ...
      offset != this.emptyBuffer.byteLength &&
        console.warn("Incorrect buffer size writed");
  ...
  */
  ...
  function normalize(model, defaultValue) {
    return align(model, function (params, prop, struct) {
      ...
      var _typed = (isArray(params) && params[0]) || params,
        ...
        length =
          (isArray(params) && params.length >= 3 && params[2]) ||
          ((isString(value) || isArray(value) || isArrayBuffer(value)) &&
            value.length) || //FIXME 0일 수도 있음
          (isArrayBuffer(value) && value.byteLength) ||
          1,
        // [3] vary, define boolean as vary type from params
        vary =
          isArray(params) && (isString(length) || length > 1)
            ? (isString(length) && params[3] === undefined && true) || params[3]
            : false;
  ...
  function arrayBufferToArray(buf, typed) {
    return Array.prototype.slice.call(
      new global[capitalize(typed) + "Array"](buf)
    );
  }

  /** support 64-bit int shim **/
  if (
    DataView.prototype.getUint64 === undefined &&
    ...
    DataView.prototype.setInt64 = function (byteOffset, value, endianness) {
